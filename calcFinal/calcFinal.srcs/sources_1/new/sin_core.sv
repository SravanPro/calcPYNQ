...
//  Computes  result = sin( mantA * 10^expA )   [radians]
...
module sin_core (
    ...
    //  Constants  (scaled by SCALE = 10^15)
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            //  Idle: latch input, kick off mantA * SCALE
            S_IDLE: begin
                done <= 1'b0;
                if (doEval) begin
                    signFlip         <= signA;
            ...
            //  Absorb expA: x10 free via shift-add, /10 via divider
            S_CONVERT: begin
            ...
            //  Quadrant reduction (all comparisons 96-bit)
            S_QUADRANT: begin
            ...
            //  xr2 = xr * xr  (raw, NOT divided by SCALE)
            //
            //  We keep it unscaled so that when x < 1.0 (xr < SCALE)
            //  the value xr*xr is not prematurely floored to zero.
            //  The two extra SCALE factors are cancelled across the
            //  two divisions in each Taylor iteration.
            S_XR2_MUL: begin
            ...
            //  denom_scaled = da * db * SCALE  (two sequential muls)
            S_DENOM_MUL: begin
                if (n < NTERMS) begin
            ...
            //
            //  Units analysis (each value carries implicit /SCALE):
            //    cur_term  ~ xr/SCALE      (1 implicit SCALE)
            //    xr2       ~ xr^2          (raw, no implicit SCALE)
            //    product   ~ xr^3/SCALE    (1 implicit SCALE)
            //    / SCALE   ~ xr^3/SCALE^2
            //    denom     = (2n)(2n+1)*SCALE
            //    / denom   ~ xr^3/SCALE^2 / ((2n)(2n+1)*SCALE)
            //              = xr^3 / ((2n)(2n+1) * SCALE^3)
            //    which is the correct fixed-point representation of
            //    x^3 / (6 * SCALE) with the /SCALE implicit.
            // -------------------------------------------------------
            S_TAYLOR_MUL: begin
            ...
            S_TAYLOR_DW2: begin
                if (div_done) begin
                    ...
                    // n=1 (odd) => subtract x^3/3!, n=2 (even) => add x^5/5!
                    if (n[0])
